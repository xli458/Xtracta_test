...
import time
import openpyxl
from bisearch import binary_search
...
with open('suppliernames.txt') as f:
...
for page in invoice_rec:
    for line in page:
        ...
        for word_tmp in line:
            line_pt += 1
            indx = binary_search(word_tmp, Sup_split_name)
            if indx >= 0:  # it is found
                name_len = len(Sup_split_name[indx])  # length of the company nmae
